# Birzhevik Brand Assets

Place real Birzhevik production assets here.

Current reference-derived PNG assets, cropped from the supplied Birzhevik identity reference images:

- `birzhevik-lockup-reference-dark.png` — blue mark plus white `Биржевик` lockup on dark background, cropped from `references/images/Youtube Preview 1.png`.
- `birzhevik-mark-reference-blue.png` — blue mark crop on dark background, cropped from `references/images/Youtube Preview 1.png`.
- `birzhevik-lockup-reference-panel.png` — white mark plus white `Биржевик` lockup on dark gradient/grid panel, cropped from `references/images/birzhevik-identity-page-1.png`.
- `birzhevik-mark-reference-blue-on-dark-tile.png` — blue mark on dark rounded tile, cropped from `references/images/birzhevik-identity-page-1.png`.

Use these real assets as logo references in GPT Image 2 requests until cleaner official SVG/PNG exports are supplied. The prompt must require the model to preserve the referenced logo exactly and unchanged. If the model distorts the logo, use these files for a narrow post-generation logo repair. Do not ask an image model to redraw or invent the official Birzhevik logo from memory.

Recommended files:

- `birzhevik-mark.svg`
- `birzhevik-mark.png`
- `birzhevik-lockup-light.svg`
- `birzhevik-lockup-dark.svg`
- `VelaSansGX.ttf`
- `Manrope.ttf`

Current repo includes rendered identity-PDF reference pages in `references/images/`; the PNGs above are reference-derived production fallbacks, not editable vector masters.
